...
public class EC2{
    ...
    void ECDSA(){
        ...
        BigInteger zero = new BigInteger( "0" ); // a value to compare 0
        do  {
            kG = multiply( G, k ); // k times G
            r = kG.x.mod( n ); // r = kG.x mod n
            s = k.modInverse( n ).multiply( message.add( privateKeyB.multiply( r ) ) ).mod( n );
            // if r or s == 0 redo k
        ...
        Point X = add( multiply( G, u1 ), multiply( publicKeyB, u2 ) );
        if ( X.equals( Point.O ) ) {
            // reject the signature
            System.out.println( "reject the signature" );
        } else if ( X.x.mod( n ).equals( r ) ) {
            // accept the signature
            System.out.println( "accept the signature" );
        } else {
            // reject the signature
            System.out.println( "reject the signature" );
        }
